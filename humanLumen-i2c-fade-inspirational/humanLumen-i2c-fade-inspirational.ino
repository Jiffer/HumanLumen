...
void loop() {
  ...
  if (stateChanged) {
    ...
    if (detector.state == NOPE) {
      ...
      if (fader1.fadeLevel > 0) {
        fader1.startFadeOff(200);
      }
      if (fader2.fadeLevel < fader2.maxLevel) {
        fader2.startFadeOn(200);
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    int h = (int)(98 * (fader1.fadeLevel/fader1.maxLevel) + (fader2.fadeLevel/fader2.maxLevel) * 120) % 255;
    int s = 255* (fader1.fadeLevel/fader1.maxLevel)+ (fader2.fadeLevel/fader2.maxLevel) * 250;
